...
#                 total_samples += batch_size
#                 if step % 10 == 0:
#                     _wandb_log({"epoch": epoch, "Loss/train_step": loss.item()})
#                     _wandb_log(self._get_monitoring_parameters())
#             avg_train_loss = total_train_loss / total_samples
#             _wandb_log({"epoch": epoch, "Loss/train": avg_train_loss})
#             eval_loss = self.eval()
#             _wandb_log({"epoch": epoch, "Loss/eval": eval_loss})
#             if eval_loss < best_eval_loss:
#                 best_eval_loss = eval_loss
#                 save_model(self.model)
#                 epochs_without_improvement = 0
#                 _wandb_log({"epoch": epoch, "Loss/best_eval": best_eval_loss})
...
def recursive_to_device(item, device):
    """
    재귀적으로 item에 포함된 모든 torch.Tensor를 지정한 device로 이동시킵니다.
    만약 item이 list나 tuple인 경우 내부 요소들에 대해 재귀적으로 호출합니다.
    """
    if isinstance(item, torch.Tensor):
        return item.to(device)
    elif isinstance(item, (list, tuple)):
        return type(item)(recursive_to_device(x, device) for x in item)
    else:
        return item
...
class Aliquot:
    ...
    def _loop(self, bat):
        bat = recursive_to_device(bat, self.device)
        y = bat[-1]
        y_hat = self.model(*bat[:-1])
        loss = self.criterion(y_hat, y)
        return loss
    ...
    def one_batch_check(self):
        ...
        for tup in self.train_dataloader:
            # print("tup",tup)
            self.optimizer.zero_grad()
            # 재귀적으로 모든 요소를 device로 이동
            tup = recursive_to_device(tup, self.device)
            y = tup[-1]
            y_hat = self.model(*tup[:-1])
            # print("DEBUG >> y_hat.shape:", y_hat.shape, "y.shape:", y.shape)
            loss = self.criterion(y_hat, y)
            loss.backward()
            self.optimizer.step()
            # Sanity check
